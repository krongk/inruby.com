class NewsCatesController < InheritedResources::Base
  ...
  def index
  	@news_items = params[:news_cate_id] ? 
  	  NewsItem.where(:news_cate_id => params[:news_cate_id]).order("updated_at DESC").paginate(:per_page => 10, :page => params[:page] || 1) : 
  	  NewsItem.order("updated_at DESC").paginate(:per_page => 8, :page => params[:page] || 1)
  end
  ...
  def show
    if params[:id] =~ /^\d+$/
      ...
      @news_items = NewsItem.where("tags regexp '#{params[:id]}' OR title regexp '#{params[:id]}'").order("updated_at DESC").paginate(:per_page => 8, :page => params[:page] || 1)
    end
